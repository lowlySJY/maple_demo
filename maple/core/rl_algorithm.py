...
class   BaseRLAlgorithm(object, metaclass=abc.ABCMeta):
    def __init__(
        ...
        self._expl_epoch_freq = expl_epoch_freq
        self._eval_only = eval_only

        self._no_training = no_training
    ...
    def _log_stats(self, epoch):
        dump_logs = False
        if not self._eval_only and epoch % self._expl_epoch_freq == 0:
            dump_logs = True
        ...
        if not self._eval_only:
            if epoch % self._expl_epoch_freq == 0:
                self._cur_expl_log = OrderedDict()
                self._cur_expl_log.update(
            ...
            logger.record_dict(self._cur_expl_log, prefix='expl/')

        """
        Evaluation
        """
        if epoch % self._eval_epoch_freq == 0:
            self._cur_eval_log = OrderedDict()
            self._cur_eval_log.update(
        ...
        logger.record_dict(self._cur_eval_log, prefix='eval/')
        ...
        try:
            ...
            k = 'process/RAM Usage (Mb)'
            v = int(process.memory_info().rss / 1000000)
            logger.record_tabular(k, v)
            logger.record_tabular('process/Num Threads', process.num_threads())
        except ImportError:
            pass

        gt.stamp('logging')
        logger.record_dict(_get_epoch_timings())
        logger.record_tabular('Dummy', 0)
